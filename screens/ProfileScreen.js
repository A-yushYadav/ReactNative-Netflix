  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "black" }}>
      ...
      <View style={{marginTop:50,alignItems:"center"}}>
        <Text style={{color:"grey",fontSize:16,fontWeight:600}}>Who's Watching</Text>
        <FlatList numColumns={2} data={profiles} renderItem={({item})=>(
          <Pressable onPress={()=>{
            ...
            <Text style={{color:"white",textAlign:"center",fontSize:15,fontWeight:"500",marginTop:10}}>{item.name}</Text>
          </Pressable>
        )}/>
      </View>

      <Pressable onPress={signOutUser}>
        <Text style={{fontSize:18,textAlign:"center",color:"grey",marginTop:15}}>Sign Out</Text>
      </Pressable>
    </SafeAreaView>
  );
};
